...
curl -fsS -X POST "$MEM0_URL/api/v1/memories" \
  -H "$AUTH" -H "$JSON" \
  -d '{"content": "smoke-test: Ian hosts services on CapRover", "agent_id": "smoke"}' && echo

echo "3. Search for it via REST"
curl -fsS -X POST "$MEM0_URL/api/v1/memories/search" \
  -H "$AUTH" -H "$JSON" \
  -d '{"query": "where does Ian host services?", "agent_id": "smoke"}' && echo
...
python "$(dirname "$0")/smoke_mcp.py" "where does Ian host services?"
